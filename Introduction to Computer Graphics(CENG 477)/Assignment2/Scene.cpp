...
/* Culling*/
bool Scene::ApplyBackfaceCulling(Vec4* firstVertex, Vec4* secondVertex, Vec4* thirdVertex, Camera camera, Vec4 transformedNormal) {
	// TODO: Done!
	...
	Vec3 normalVector = { transformedNormal.x,transformedNormal.y,transformedNormal.z, transformedNormal.colorId};

	Vec3 viewVector = normalizeVec3(subtractVec3(firstVertexVec3, camera.pos));
...
{   /*
    ...
    */
	...
	for(Model *model : models){
		...
		vector<Vec4> transformedNormals;
		...
		for(Triangle triangle: model->triangles){
			...
			Vec3 normalVector = normalizeVec3(crossProductVec3(subtractVec3(secondVertex, firstVertex),
				subtractVec3(thirdVertex, firstVertex)));
			Vec4 normalVector4 = { normalVector.x,normalVector.y,normalVector.z,1,normalVector.colorId };

			Matrix4 a = multiplyMatrixWithMatrix(getIdentityMatrix(), totalMatrix);

			double values[16] = {
				a.val[0][0],a.val[1][0] ,a.val[2][0] ,a.val[3][0] ,
				a.val[0][1],a.val[1][1] ,a.val[2][1] ,a.val[3][1] ,
				a.val[0][2],a.val[1][2] ,a.val[2][2] ,a.val[3][2] ,
				a.val[0][3],a.val[1][3] ,a.val[2][3] ,a.val[3][3]
			};
			 
			Vec4 transformedNormal = multiplyMatrixWithVec4(gluInvertMatrix(values, values), normalVector4);
        ...
        if(this->cullingEnabled){
            ...
            while(i<modelVertices.size()) {
                bool isCulled = ApplyBackfaceCulling(&modelVertices[i], &modelVertices[i+1], &modelVertices[i+2], *camera, transformedNormals[i/3]);
                if(isCulled){
                    modelVertices.erase(modelVertices.begin()+i+2);
                    modelVertices.erase(modelVertices.begin()+i+1);
                    modelVertices.erase(modelVertices.begin()+i);
